...
function DomainTab({ apiBaseUrl }) {
  ...
  const token = localStorage.getItem('token')
  const API = apiBaseUrl || 'http://localhost:8000'
...
http {
    ...
    limit_req_zone $binary_remote_addr zone=redirect:10m rate=120r/m;
...
curl -sf "${BACKEND_URL}/api" >/dev/null 2>&1 && { echo "✅ Backend OK";  BACKEND_OK=1;  } || { echo "⚠️  Backend not ready"; $DOCKER_COMPOSE logs --tail=20 backend; }
curl -sf "${FRONTEND_URL}/"   >/dev/null 2>&1 && { echo "✅ Frontend OK"; FRONTEND_OK=1; } || echo "⚠️  Frontend not ready"
...
✅ NEW in v11.8.1 — Bug fixes:
  🐛 FIXED: Backend crash — click_events relationship now on both Link and Click models
  🐛 FIXED: Backend race crash on startup — 2-worker collision on create_all (now 1 worker)
  🐛 FIXED: NGINX routing /@vite/client to backend (502) — Vite HMR paths explicitly go to frontend
  🐛 FIXED: Port :80 blank — caused by all 3 bugs above
...
🌐 NGINX runs on port 80 — use http://YOUR_IP for public access
   API docs: ${BACKEND_URL}/docs
