...
// ✅ FIX: Default to 'dark'. Use a ref-based toggle to avoid React 18
// StrictMode double-invoking the functional updater (light→dark→light flicker).
...
// ✅ FIX: Read current value directly — avoids functional-updater double-invoke
// in React 18 StrictMode which caused dark→light revert on first click.
...
// ✅ FIX: Detect external URLs so they open correctly instead of being
// treated as internal React Router paths (which would just show a 404).
...
{visibleItems.map(item => {
  ...
  return external ? (
    // ✅ FIX: External URL — use plain <a> with target="_blank"
...
// ✅ FIX: Helper to detect external URLs so the form can show contextual hints
...
<form onSubmit={handleSubmit} style={{display:'grid',gap:'1rem'}}>
  {/* ✅ Label + URL side by side on wider screens */}
  <div style={{display:'grid',gap:'1rem',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))'}}>
    ...
    <div>
      <label style={{display:'block',marginBottom:'.35rem',fontWeight:500,fontSize:'.875rem'}}>
        {/* ✅ FIX: Dynamic label so admins know both options */}
        URL / Path *
        {external
...
🛠️ V10.5 FIXES:
  ✅ Dark mode is now DEFAULT (no more light flash on first load)
  ✅ Dark mode toggle no longer auto-reverts — fixed React 18 StrictMode
     double-invoke bug in the functional state updater
  ✅ Nav Manager links now work — enter any full URL (https://example.com)
     to link externally (opens new tab) or /path for internal routes
  ✅ Nav Manager form shows live hint: "🌐 External link" vs "🔗 Internal route"
  ✅ Nav table now shows 🌐 prefix for external URLs so admins can tell at a glance
