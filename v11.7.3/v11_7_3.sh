...
export BACKEND_PORT FRONTEND_PORT BACKEND_URL FRONTEND_URL
...
class User(Base):
    ...
    header_image_height = Column(Integer, default=30)
...
def update_bio_profile(profile: dict, db: Session = Depends(get_db), current_user=Depends(auth.get_current_active_user)):
    fields = [
        ...
        'profile_layout','profile_photo_style','header_image_height'
    ]
    for field in fields:
...
async def get_public_profile(request: Request, slug: str, db: Session = Depends(get_db)):
    ...
    config = {
        ...
        "FRONTEND_URL": settings.FRONTEND_URL,  # Added for report link
    }
    templates = Jinja2Templates(directory=os.path.join(os.path.dirname(__file__), "..", "templates"))
    # Inject helper functions into template environment
...
.hero {
  ...
  overflow: hidden;  /* clip banner to card border-radius */
}
.hero::before {
  content: ''; position: absolute; top: 0; left: 0; width: 100%; height: var(--banner-h, 30%);
  background: linear-gradient(135deg, var(--primary), var(--secondary));
  opacity: 0.1; z-index: 0; border-radius: var(--r-md) var(--r-md) 0 0;
}
.hero-banner {
  position: absolute; top: 0; left: 0; width: 100%; height: var(--banner-h, 30%);
...
<main>
  ...
  <header class="hero" style="--banner-h: {{ profile.header_image_height or 30 }}%">
    ...
    <div class="hero-content">
      ...
      <!-- Photo row: @slug LEFT · avatar CENTER · status bubble RIGHT -->
      <div class="photo-row">
        {% if profile.custom_slug %}
        <div class="slug-vertical">@{{ profile.custom_slug }}</div>
        {% endif %}
        ...
        <div class="avatar-wrap">
          ...
          <img src="{{ profile.profile_photo_url }}" alt="{{ profile.header_text or profile.custom_slug }}"
               class="avatar {{ profile.profile_photo_style }}"
  ...
  <footer class="footer">
    ...
    <p style="position:relative;z-index:2">
      ...
      V{{ config.SITE_VERSION or '11.7.2' }}
    </p>
  </footer>
</main>
...
def run_migrations():
    migrations = [
        ...
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS header_image_height INTEGER DEFAULT 30",
    ]
...
app = FastAPI(title=settings.SITE_NAME, version="11.7.2")
...
@app.get("/")
def root():
    return {"message": f"{settings.SITE_NAME} API v11.7.2", "docs": "/docs"}
...
{"name":"link-platform","version":"11.7.2","type":"module","scripts":{"dev":"vite","build":"vite build","preview":"vite preview"},"dependencies":{"react":"^18.2.0","react-dom":"^18.2.0","react-router-dom":"^6.20.0","axios":"^1.6.0","qrcode.react":"^3.1.0"},"devDependencies":{"@vitejs/plugin-react":"^4.1.0","vite":"^4.5.0"}}
EOF
...
import Navbar from '../components/Navbar'
import Toast from '../components/Toast'
import { SOCIAL_PLATFORMS, TAB_TYPES, TAB_STYLES, HEADER_STYLES, THEME_PRESETS, API_BASE_URL } from '../config'
...
export default function BioProfile() {
  ...
  const [profile, setProfile] = useState({
    ...
    profile_layout: 'center', profile_photo_style: 'circle', header_image_height: 30
  })
            ...
            <div style={{ display: 'grid', gap: '2rem' }}>
              ...
              <div className="glass" style={{ padding: '1.5rem', display: 'grid', gap: '1.25rem' }}>
                ...
                {profile.header_image_url && (
                  <div>
                    <label style={{ fontWeight: 500, display: 'block', marginBottom: '.5rem' }}>
                      🖼️ Banner Height: <span style={{ color: 'var(--primary)', fontWeight: 700 }}>{profile.header_image_height || 30}%</span>
                    </label>
                    <input
                      type="range" min="5" max="100" step="5"
                      value={profile.header_image_height || 30}
                      onChange={e => setProfile({ ...profile, header_image_height: parseInt(e.target.value) })}
                      style={{ width: '100%', accentColor: 'var(--primary)', cursor: 'pointer' }}
                    />
                    <div style={{ display: 'flex', gap: '.4rem', marginTop: '.6rem', flexWrap: 'wrap' }}>
                      {[['Peek', 15], ['Standard', 30], ['Half', 50], ['Large', 75], ['Full', 100]].map(([lbl, val]) => (
                        <button
                          key={val} type="button"
                          onClick={() => setProfile({ ...profile, header_image_height: val })}
                          style={{
                            padding: '.3rem .7rem', borderRadius: '.5rem', fontSize: '.75rem', fontWeight: 600,
                            border: `1.5px solid ${(profile.header_image_height || 30) === val ? 'var(--primary)' : 'var(--border)'}`,
                            background: (profile.header_image_height || 30) === val ? 'rgba(167,139,250,.15)' : 'transparent',
                            color: (profile.header_image_height || 30) === val ? 'var(--primary)' : 'var(--text-muted)',
                    ...
                    <p style={{ fontSize: '.72rem', color: 'var(--text-muted)', marginTop: '.4rem' }}>
                      Drag the slider or pick a preset. 100% fills the entire profile card with your banner.
                    </p>
                  </div>
                )}
              </div>
...
✅ NEW: Header banner height is now user-controlled (5%–100%)
✅ Drag a slider or pick Peek/Standard/Half/Large/Full presets
✅ 100% fills the entire profile card with your banner image
✅ banner height persists per-user in the database
✅ Vertical @slug on LEFT of profile photo (animated float)
✅ Daily status bubble on RIGHT with cute arrow pointer
✅ Theme Selector in Bio Editor → pick from 5 pre-made themes + Custom CSS
✅ NO auto-rotating themes — users pick what they want, it stays!
✅ All previous features: 2FA, ban/suspend, messages, password reset preserved
...
🛠️ Commands:
  ...
  $DOCKER_COMPOSE down -v && bash v11.7.3.sh   # full reset

FINALMSG

[ $BACKEND_OK -eq 0 ] && echo "⏳ If backend not up yet: $DOCKER_COMPOSE logs backend"