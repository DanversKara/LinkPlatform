...
<main>
  ...
  <footer class="footer">
    ...
    <p style="position:relative;z-index:2">
      ...
      V{{ config.SITE_VERSION or '11.7.4' }}
    </p>
  </footer>
</main>
...
✅ Smart Link Types — Email (mailto:), Phone (tel:), Address (click-to-copy), Video Embed, Image
✅ Tab Background Opacity — FIXED: slider now correctly controls image visibility (like Header Image Opacity)
✅ Tab Style — FIXED: Solid/Glass/Frost/Transparent now work correctly WITH and WITHOUT a background image
✅ Tab Text Color — NEW: per-tab color picker overrides theme text color (fix white-on-white / black-on-black)
✅ @Slug Click-to-Copy — visitors click the @slug to copy it to clipboard
✅ @Slug Display Styles — Vertical Rotated, Vertical Straight, Horizontal, Hidden
✅ Header Image Size — Half Banner, Full Banner, or Cover (full bleed background)
✅ Header Image Opacity — slider to control how visible the banner image is
