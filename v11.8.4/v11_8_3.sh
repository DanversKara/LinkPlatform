...
def verify_my_domain(request: Request, db: Session = Depends(get_db), current_user=Depends(auth.get_current_user),
    ...
    # Force/self-approve: skip DNS check, mark verified immediately.
    # Intended for Cloudflare-proxied domains where the IP check can't work.
    # The domain still needs to actually route traffic to this server — if it
    # doesn't, the user's links simply won't work, which is self-correcting.
    if force:
...
// When API_BASE_URL is empty (relative mode), use the browser's current origin
// so short URLs display as full clickable URLs (e.g. https://mysite.com/s/abc123)
// rather than just /s/abc123
export const getOrigin = () => (typeof window !== 'undefined' ? window.location.origin : "")
export const shortUrl  = (code) => `${API_BASE_URL || getOrigin()}/s/${code}`
export const landingUrl = (code) => `${API_BASE_URL || getOrigin()}/l/${code}`
...
function DomainTab({ apiBaseUrl }) {
  ...
  const verify = (force = false) => {
    setVerifying(true)
    const url = force
      ? `${API}/api/domains/my/verify?force=true`
      : `${API}/api/domains/my/verify`
    fetch(url, { method: 'POST', headers: { Authorization: `Bearer ${token}` } })
      .then(async r => {
        setVerifying(false)
        if (!r.ok) { const e = await r.json(); return showToast(e.detail || 'DNS not ready yet', 'error') }
        showToast(force ? '✅ Domain approved! Links will work once DNS/Cloudflare is live.' : '✅ Verified and live!')
        load()
      })
  }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: '1.5rem' }}>
      ...
      <div className="glass" style={{ padding: '1.5rem', borderRadius: 12 }}>
        ...
        <div style={{ display: 'grid', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '.75rem', flexWrap: 'wrap' }}>
            ...
            {record && !record.is_verified && <button className="btn btn-outline" onClick={() => verify(false)} disabled={verifying}>{verifying ? 'Checking…' : '🔍 Verify DNS'}</button>}
            {record && !record.is_verified && (
              <button className="btn btn-outline" onClick={() => {
                if (confirm('Mark this domain as active without DNS verification?\n\nUse this if you\'re using Cloudflare proxy (orange cloud) and automatic detection isn\'t working.\n\nYour links will only work once your domain is actually pointing to this server.'))
                  verify(true)
              }} disabled={verifying} style={{ borderColor: '#f59e0b', color: '#f59e0b' }}>
      ...
      <div className="glass" style={{ padding: '1.5rem', borderRadius: 12 }}>
        ...
        <div style={{ marginTop: '.75rem', padding: '.75rem 1rem', background: 'rgba(245,158,11,.08)', borderRadius: 8, borderLeft: '3px solid #f59e0b', fontSize: '.82rem', lineHeight: 1.6 }}>
          ...
          If <strong>Verify DNS</strong> doesn't work (Cloudflare masks your IP), click <strong>I'm using Cloudflare</strong> to self-approve — your links will activate as soon as your domain is pointed at this server.
        </div>
      </div>
    </div>
  )
...
✅ NEW in v11.8.3 — Deploy Domain support + critical bug fixes:
  ...
  ⚡ Vite allowedHosts: true — correct Vite 4.x boolean, allows all hosts safely behind NGINX
  🐛 FIXED: Short links showed /s/code instead of full URL — shortUrl() now uses window.location.origin
  🐛 FIXED: Admin/users couldn't create links — POST /api/links missing trailing slash caused 307 redirect losing POST body
  🔶 FIXED: Cloudflare proxied domains stuck on Pending — auto-detect + "I'm using Cloudflare" self-approve button
  🐛 FIXED: 503 on dashboard load — NGINX api rate limit was 30r/m; now 30r/s with burst=50
