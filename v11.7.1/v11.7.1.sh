...
SITE_VERSION="11.7.1"  # Updated version
...
# ---------- Templates: public_profile.html (UPDATED with new layout and frontend report link) ----------
...
<head>
    ...
    <style>
        ...
        /* Photo row with vertical slug, photo, thought bubble */
        .photo-row {
            display: flex;
            align-items: center;
            gap: 1rem;
        ...
        .thought-bubble::after {
            ...
            border-right: 10px solid var(--surface2);
        }
        .bubble-empty {
            display: none;
        }
    ...
    <div class="profile-card">
        ...
        <div class="photo-row">
            ...
            <div class="photo-wrapper">
                ...
                {% else %}
                    <div class="profile-photo" style="background: var(--surface2); display: flex; align-items: center; justify-content: center;">
        ...
        <!-- Report button inside card — now points to frontend -->
        <div style="text-align: center;">
            <a href="{{ config.FRONTEND_URL }}/report?slug={{ profile.custom_slug }}" class="report-link" target="_blank">🚩 Report Profile</a>
        </div>
    </div>

    <!-- Site powered by footer (outside card) -->
    <div class="site-footer">
        {% set site_name = config.SITE_NAME or 'LinkPlatform' %}
        Powered by <a href="{{ base_url }}">{{ site_name }}</a>
    </div>
...
app = FastAPI(title=settings.SITE_NAME, version="11.7.1")  # Updated version
...
@app.get("/")
def root():
    return {"message": f"{settings.SITE_NAME} API v11.7.1", "docs": "/docs"}  # Updated version
...
{"name":"link-platform","version":"11.7.1","type":"module","scripts":{"dev":"vite","build":"vite build","preview":"vite preview"},"dependencies":{"react":"^18.2.0","react-dom":"^18.2.0","react-router-dom":"^6.20.0","axios":"^1.6.0","qrcode.react":"^3.1.0"},"devDependencies":{"@vitejs/plugin-react":"^4.1.0","vite":"^4.5.0"}}
EOF
...
🛠️ V11.7.1 NEW FEATURES & FIXES:
  ✅ Redesigned public profile page with:
     - Vertical slug on the left of profile photo
     - Thought‑bubble daily status on the right
     - Header, sub‑header, bio below
     - Report button at bottom of card (now links to FRONTEND)
     - "Powered by" footer outside the card
  ✅ All previous features (2FA, ban/suspend, message privacy, password reset) preserved.
  ✅ FIXED: API routes in public.py now correctly ordered before the catch‑all profile route – report‑profile works!
  ✅ FIXED: Report link now correctly points to frontend (http://localhost:3000/report)
  ✅ REMOVED: Backup/restore logic – now a clean fresh install every time.

🛠️ COMMANDS:
  $DOCKER_COMPOSE logs -f backend
  $DOCKER_COMPOSE logs -f frontend
  $DOCKER_COMPOSE restart
  $DOCKER_COMPOSE down -v && bash v11.7.1.sh   # full reset (fresh install)
